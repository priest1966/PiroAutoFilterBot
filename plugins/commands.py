    ...
    if data.startswith("all"):
        ...
        for file in files:
            ...
            await client.send_cached_media(
                ...
                reply_markup=InlineKeyboardMarkup( [ [ InlineKeyboardButton('MovieVerse', url=f"https://t.me/movieversepremium") ] ] ),
            )
        return
    ...
    if not files_:
        ...
        try:
            msg = await client.send_cached_media(
                ...
                file_id=file_id,
                protect_content=True if pre == 'filep' else False,
                reply_markup=InlineKeyboardMarkup( [ [ InlineKeyboardButton('MovieVerse', url="https://t.me/movieversepremium") ] ] ),
            )
            filetype = msg.media
    ...
    if settings is not None:
        buttons = [
            [
                ...
                InlineKeyboardButton(
                    'Single Button' if settings["button"] else 'Double',
                    callback_data=f'setgs#button#{settings["button"]}#{grp_id}',
                ),
            ],
            [
            ...
            [
                ...
                InlineKeyboardButton(
                    '𝖮n' if settings["file_secure"] else '𝖮ff',
                    callback_data=f'setgs#file_secure#{settings["file_secure"]}#{grp_id}',
                ),
            ],
            [
            ...
            [
                ...
                InlineKeyboardButton(
                    '5 Min' if settings["auto_delete"] else 'Off',
                    callback_data=f'setgs#auto_delete#{settings["auto_delete"]}#{grp_id}',
                ),
            ],
            [
    ...
    if message.reply_to_message:
        ...
        try:
            ...
            if str(user.id) in str(out):
                ...
                success = False
            if success:
                await message.reply_text(f"<b>Your Message Has Successfully Sent To {user.mention}.</b>")
            else:
                await message.reply_text("<b>An Error Occured !</b>")
        except Exception as e:
            await message.reply_text(f"<b>Error :- <code>{e}</code></b>")
    else:
        await message.reply_text("<b>Error Command 𝖨𝗇𝖼𝗈𝗆𝗉𝗅𝖾𝗍𝖾 !</b>")
